...
export const Chip = styled<IChipProps, 'span'>('span')`
  ...
  white-space: nowrap;
  ${unselectable};
  ${(props: IChipProps) => props.color && css`
    background-color: ${props.color};
    color: ${getInverse(props.color)};
  `}
  ${(props: IChipProps) => props.size && css`
    ${getFontSize(props.size)}
  `}
  ${(props: IChipProps) => css`
    &:hover, &:focus {
      background-color: ${darken(.05, props.color || tagBackgroundColor)};
    }
    &:active {
      background-color: ${darken(.1, props.color || tagBackgroundColor)};
    }
  `}
  ${(props: IChipProps) => props.isRounded && css`
...
export const Chips = styled<IChipsProps, 'div'>('div')`
  ...
  ${(props: IChipsProps) => props.alignment === 'center' && css`
    justify-content: center;
  `}
  ${(props: IChipsProps) => props.alignment === 'right' && css`
    justify-content: flex-end;
  `}
  & ${/* sc-selector */ Chip} {
    ${(props: IChipsProps) => props.hasAddons && css`
      margin-right: 0;
    `}
    ${(props: IChipsProps) => props.alignment === 'center' && css`
      margin-right: 0.25em;
