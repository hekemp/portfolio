...
export const Columns = styled<IColumnsProps, 'div'>('div')`
  ...
  ${(props: IColumnsProps) => props.alignment === 'center' && css`
    justify-content: center;
  `}
  ${(props: IColumnsProps) => props.isGapless && css`
    margin-left: 0;
    margin-right: 0;
    margin-top: 0;
    & > ${/* sc-selector */ Column} {
  ...
  `}
  ${(props: IColumnsProps) => props.isMultiline && css`
    flex-wrap: wrap;
  `}
  ${(props: IColumnsProps) => props.isVcentered && css`
    align-items: center;
  `}
  ${media.tablet`
    display: flex;
